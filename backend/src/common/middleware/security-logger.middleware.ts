...
export class SecurityLoggerMiddleware implements NestMiddleware {
  ...
  use(req: Request, res: Response, next: NextFunction) {
    ...
    if (originalUrl.includes('../') || originalUrl.includes('/etc/passwd')) {
       this.logger.error(`Suspicious Access Attempt from IP: ${ip} for URL: ${originalUrl} [User Agent: ${userAgent}]`);
    }

    res.on('finish', () => {
      const { statusCode } = res;
      if (statusCode >= 400) {
        this.logger.warn(`${method} ${originalUrl} ${statusCode} - IP: ${ip} - User-Agent: ${userAgent}`);
      }
    });
