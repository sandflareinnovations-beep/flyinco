...
import { PiChartBar, PiFileArrowDown, PiUsers, PiMagnifyingGlass, PiCaretDown, PiCaretUp } from "react-icons/pi";
...
export default function StaffReportsPage() {
    ...
    const totals = filtered.reduce(
        (acc: any, r: any) => ({
            ...
            seats: acc.seats + (r.totalSeats || 0),
            remaining: acc.remaining + (r.remainingSeats || 0),
            sold: acc.sold + (r.soldSeats || 0),
        }),
        { bookings: 0, seats: 0, remaining: 0, sold: 0 }
    );
        ...
        <div className="space-y-6 max-w-7xl">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-0.5">Flight-wise passenger lists & booking breakdown.</p>
                </div>
                <Button className="bg-violet-600 hover:bg-violet-700 text-white rounded-xl gap-2 font-semibold" onClick={handleExportAll}>
            ...
            <div className="grid grid-cols-4 gap-4">
                {[
                    { label: "Total Bookings", value: totals.bookings.toString(), icon: PiUsers, color: "text-violet-600 bg-violet-50" },
                    { label: "Total Seats", value: totals.seats.toString(), icon: PiChartBar, color: "text-blue-600 bg-blue-50" },
                    { label: "Seats Sold", value: totals.sold.toString(), icon: PiChartBar, color: "text-emerald-600 bg-emerald-50" },
                    { label: "Remaining", value: totals.remaining.toString(), icon: PiChartBar, color: "text-amber-600 bg-amber-50" },
                ].map((card) => (
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="bg-gray-50 border-gray-100">
                            ...
                            <TableHead className="font-bold text-xs text-gray-500 text-center">Bookings</TableHead>
                        ...
                        {filtered.length === 0 ? (
                            ...
                            filtered.map((route: any) => (
                                <>
                                    ...
                                    >
                                        ...
                                        <TableCell className="text-center">
                                            <div className="text-xs space-y-0.5">
                                                <div className="font-bold text-gray-700">
                                                    {route.soldSeats}/{route.totalSeats}
                                                </div>
                                                <div className="text-gray-400">{route.remainingSeats} remaining</div>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-center">
                                            <div className="text-xs space-y-0.5">
                                                <div className="font-bold text-gray-700">{route.totalBookings}</div>
                                                <div className="text-gray-400">
                                                    {route.confirmedBookings} cnf · {route.heldBookings} held
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                >
                                                    {loadingPassengers === route.routeId ? (
                                                        "Loading..."
                                                    ) : (
                                                        <>
                                    ...
                                    {/* Expandable Passenger List - No financial columns */}
                                    <AnimatePresence>
                                                    ...
                                                    >
                                                        <div className="p-4">
                                                            <div className="flex items-center gap-3 mb-3">
                                                                <p className="text-xs font-black text-violet-900 uppercase tracking-wider">
                                                                    Passenger List
                                                                </p>
                                                                ...
                                                                <table className="w-full text-xs">
                                                                    <thead>
                                                                        <tr className="bg-violet-50 border-b border-violet-100">
                                                                            ...
                                                                            <th className="text-left px-3 py-2 font-bold text-violet-800">Ticket No</th>
                                                                            ...
                                                                            <th className="text-left px-3 py-2 font-bold text-violet-800">Phone</th>
                                                                        </tr>
                                                                    </thead>
                                                                    <tbody>
                                                                        {passengerData[route.routeId].passengers.map((p: any) => (
                                                                            <tr key={p.id} className="border-b border-gray-50 hover:bg-violet-50/30">
                                                                                ...
                                                                                <td className="px-3 py-2 font-mono text-violet-700 font-bold">
                                                                                    {p.pnr || "---"}
                                                                                </td>
                                                                                <td className="px-3 py-2 font-mono text-gray-500">
                                                                                    {p.ticketNumber || "---"}
                                                                    ...
                                                                    <tfoot>
                                                                        <tr className="bg-violet-50 border-t border-violet-200 font-bold">
                                                                            <td colSpan={8} className="px-3 py-2 text-violet-800">
                                                                                TOTAL ({passengerData[route.routeId].summary.totalPassengers} pax)
                                                                            </td>
                                                                        </tr>
                                                                    </tfoot>
                                                                </table>
